...
def get_fastchem_symbol(species_name):
    if species_name == 'Fe': return 'Fe'
    if species_name == 'Fe+': return 'Fe1+'
    if species_name == 'FeH': return 'Fe1H1'
    if species_name == 'H2': return 'H2'
    if species_name == 'He': return 'He'
    if species_name == 'H': return 'H'
    if species_name == 'H-': return 'H1-'
    if species_name == 'e-': return 'e-'
...
def create_model(params, spectrum_type, do_contribution, new_atmo, atmosphere='null', ptprofile = 'guillot'):
    ...
    if abundances['H2'] >= 0.0:
        ...
        fastchem = pyfastchem.FastChem('pyfastchem/input/element_abundances_10x_solar.dat', 'pyfastchem/input/logK.dat', 1)
        ...
        gas_number_density = pressures*1e6 / (nc.kB * temperature)
        for key in abundances:
            fastchem_key = get_fastchem_symbol(key)
            fastchem_index = fastchem.getGasSpeciesIndex(fastchem_key)
            abundances[key] = number_densities[:,fastchem_index]/gas_number_density
            
    MMW = 2.33 * np.ones_like(temperature) #not sure if FastChem calculates this or not, check later
