...
def convert_fits(inname, velocity_offset):
    ...
    try:
        doppler_shift = 1.0 / (1.0 - ((radvel + hdu[0].header['OBSVEL'])/1000. + velocity_offset) * (-1.0) / ckms)
    except KeyError:
        doppler_shift = 1.0 / (1.0 - ((radvel + hdu[0].header['SSBVEL'])/1000. + velocity_offset) * (-1.0) / ckms)
...
def get_velocity_offset(inname):
    ...
    #os.system('esorex --recipe-config=wasp76_test.rc molecfit_model ' + sof_file)
...
def run_molecfit(planet_name, observation_epoch):
    ...
    if float(observation_epoch[0:4]) >= 2019:
        ...
        data_location = path + observation_epoch + '_' + planet_name + '/*' + arm_file + '*.dxt.avr'
    ...
    for spectrum_file in spectra_files:
        ...
        if planet_name == 'TOI-1431b':
            rc1file = 'Default_model_narrowline.rc'
        else:
            rc1file = 'Default_model.rc'

        os.system('esorex --recipe-config=' + rc1file + ' molecfit_model ' + sof_file)
